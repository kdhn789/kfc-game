...
module.exports = {
    ...
    onQSkill: (p, room, socketId) => {
        ...
        // 자신의 HP가 30 이하이거나 스킬 사용 시 사망하는 경우 사용 불가 및 경고 메시지
        if (p.hp <= 30) {
            p.dialogue = "이러다 곧 뒤질 것 같아서 사용할 수 없습니다!";
            p.dialogueTimer = 90;
            return;
        }
    ...
    // SHIFT(R) 스킬: 폐 터뜨리기 (기존 Q에서 SHIFT로 이동 + 근접 범위 공격 8 추가)
    onRSkill: (p, room, socketId) => {
        ...
        for (let id in room.players) {
            if (id !== socketId) {
                ...
                // 폐스킬 사용시 근접 범위 공격 8 추가 적용
                const attackBox = {
                    x: p.facing === 'right' ? p.x : p.x - 40,
                    y: p.y,
                    width: p.width + 40,
                    height: p.height
                };
